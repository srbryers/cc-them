...
1. **Select your approach** — Declare complexity: Driver (risky/new), Long Iron (multi-package), Short Iron (standard), Wedge (small), Putter (trivial)
2. **Check the codebase** — Review relevant codebase sections for files you'll modify
3. **Scan for hazards** — Check known hazards from recent scorecards and common issues for known gotchas
...
1. **Score the shot** — Determine result: Fairway (clean start), Green (landed correctly), In the Hole (perfect), or miss direction (over/under/wrong approach/drift)
...
1. **Score the sprint** — Audit commits, compute final score vs par
